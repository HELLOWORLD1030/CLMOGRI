# 使用QS模型训练GEEK的embedding数据(由metapath2vec)
import os.path
...
class scmti:
    def __init__(self, cluster, save, seed):
        ...
        self.pkl_file = f'../Data/dataset_pkl/dataset_train_save_GM12878_metapath2vec_{self.cluster}.pkl'
        ...
        self.GM12878_OUTPUT_FOLDER = os.path.join(self.OUTPUT_FOLDER, "GM12878_metapath")
        self.run_count_file = "../run_count"
        self.run_count = 13  # 运行次数

        self.cluster = cluster
        self.device = torch.device("cuda:0")
        self.path_embeddings = os.path.join(self.DATA_FOLDER,"embedding","GM12878","metapath2vec", f'metapath2vec_{self.cluster}_embedding.csv')
        # self.path_embeddings = os.path.join(self.GM12878_OUTPUT_FOLDER, f'{self.cluster}_embedding.csv')
        ...
        self.seed = seed
        self.pearson = 0.7
        self.result_path = f'../Result/result_GM12878_metapath2vec_{self.seed}_{self.run_count}.xlsx'
    ...
    def dataset_preparation(self, path_output, path_label, pearson):
        ...
        node_Regu[0] = node_Regu[0].str.replace("v.", "")
        node_Regu = node_Regu[0]
        ...
        for item in location_df.iterrows():
            ...
            if len(loca[0]) != 0 and len(loca_next[0]) != 0:
                ...
                if ui.index[loca[0]][0] in node_Gene.values:  # 基因
                    ...
                    class_A = list([0])  #
                elif ui.index[loca[0]][0] in node_Regu.values:
                    class_A = list([1])  # 调控子
                if ui.index[loca_next[0]][0] in node_Gene.values:  # 基因

                    class_B = list([0])
                elif ui.index[loca_next[0]][0] in node_Regu.values:
                    class_B = list([1])
    ...
    def train(self, epochs):
        ...
        main_net_scheduler = torch.optim.lr_scheduler.StepLR(main_net_optimizer, step_size=5,
                                                             gamma=0.99)
        ...
        for epoch in range(epochs):
            ...
            for i, batch in enumerate(trainLoader):
                ...
                print(output_edge.argmax(-1).cpu().detach().numpy().round())
            ...
            Edge_loss_train = Edge_loss.cpu().detach().numpy()
            BCE_loss_train = BCE_loss.cpu().detach().numpy()
            print(loss_train, AUROC_train, f1_train)
            print(f" bce:{BCE_loss_train}, Edge_loss:{Edge_loss_train}")
            ...
            print('aupr_node:', aupr_node_test, 'acc:', acc_test, 'Distance:', distance_test, 'aupr_edge:',
                  aupr_edge_test)
            if AUROC_test > best:
                torch.save(main_net, f=f'../Output/model/model_CLMOGRI_GM12878_metapath2vec_{cluster}.pth')
                best = AUROC_test
            print('best:', best)
