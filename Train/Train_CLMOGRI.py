...
class scmti:
    def __init__(self,cluster,save,seed):
        self.run_count = 10# 运行次数
        # self.set_run_count()
        self.cluster = cluster
        self.device = torch.device("cuda:0")
        self.path_embeddings = f'../Data/embedding/CLMOGRI/{self.cluster}_embeddings_v3.csv'
        ...
        self.path_Gene = f'/{self.cluster}_allGenes.txt'
        self.path_Regu = f'/{self.cluster}_allregulators.txt'
        self.path_label = f'/{self.cluster}_network.txt'
        self.seed = seed
        self.pearson = 0.8
        self.result_path = f'../Result/result_CLMOGRI_node2vec_{self.seed}_{self.run_count}.xlsx'
        self.pkl_file = f'../Data/dataset_pkl/dataset_train_save_CLMOGRI_node2vec_{self.cluster}.pkl'
        ...
        self.sheet_headers = ["epoch","random_seed","loss_info_nce","train_acc","train_aupr","train_auroc","train_f1","test_acc",
    ...
    def train(self, epochs):
        ...
        main_net_optimizer = torch.optim.Adam(main_net.parameters(), lr=1e-3)
        ...
        # for i in tqdm(range(epochs)):
            ...
            f1_train = f1_score(label_edge.to(torch.float32).cpu().detach().numpy(), output_edge.argmax(-1).cpu().detach().numpy().round(), average='binary')
            ...
            print('aupr_node:', aupr_node_test, 'acc:', acc_test, 'Distance:', distance_test, 'aupr_edge:',
                  aupr_edge_test)
            if AUROC_test > best:
                torch.save(main_net, f=f'../Output/model/model_CLMOGRI_node2vec_{cluster}.pth')
